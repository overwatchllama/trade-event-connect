...
grouped_logic = """
          <div className="max-h-[60vh] overflow-y-auto space-y-4 pr-1">
            {(() => {
              const groups: Record<string, { preset: string | null; created_at: string; itemIds: Set<string>; labelCount: number; reprintCount: number; itemsCount: number }> = {};
              rows.forEach(r => {
                const d = new Date(r.created_at);
                const key = `${r.preset}-${d.getFullYear()}-${d.getMonth()}-${d.getDate()}-${d.getHours()}-${d.getMinutes()}`;
                if (!groups[key]) {
                  groups[key] = { preset: r.preset, created_at: r.created_at, itemIds: new Set<string>(), labelCount: 0, reprintCount: 0, itemsCount: 0 };
                }
...
# Find the list rendering block and replace it
...
# Using a simpler replacement based on the div class
pattern = r'<div className="max-h-\[60vh\] overflow-y-auto divide-y">.*?</div>\n\s+\)\s+}'
