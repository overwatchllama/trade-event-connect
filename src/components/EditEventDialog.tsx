...
const EditEventDialog = ({ open, onOpenChange, eventId, onEventUpdated }: EditEventDialogProps) => {
  ...
  const fetchEventData = async () => {
    try {
      ...
      if (!socialError && socialMedia && socialMedia.length > 0) {
        setSocialMediaLinks(socialMedia.map(sm => ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let floorPlanUrl = floorPlanPreview;
      ...
      if (socialMediaLinks.some(link => link.platform && link.url)) {
        ...
        if (socialError) {
          console.error('Error updating social media links:', socialError);
        }
      }
                  ...
                  {sponsorTiers.map((tier, index) => (
                    <div key={index} className="border rounded p-2 space-y-2">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center space-x-2">
                          <Switch id={`edit-unlimited-${index}`} checked={tier.unlimitedSlots} onCheckedChange={(checked) => updateSponsorTier(index, 'unlimitedSlots', checked)} disabled={noSponsors} />
                          <Label htmlFor={`edit-unlimited-${index}`} className="text-xs">Unlimited</Label>
                        </div>
                      </div>
                    </div>
                  ))}
