...
interface VendorStaffRosterProps {
  vendorId: string;
}

const VendorStaffRoster = ({ vendorId }: VendorStaffRosterProps) => {
  ...
  const fetchAll = async () => {
    try {
      ...
      if (fetchedRoles.length === 0) {
        const defaults = ["Event Staff", "Cashier", "Setup/Teardown", "Manager", "Sales"];
        const { data: seeded } = await supabase
          .from("vendor_staff_roles")
          .insert(defaults.map((r) => ({ vendor_id: vendorId, role_name: r })))
          .select("id, role_name");
        setRoles(seeded || []);
      } else {
      ...
      console.error("Error fetching vendor staff roster:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const addMember = async () => {
    ...
    try {
      const { error } = await supabase.from("vendor_staff_roster").insert({
        vendor_id: vendorId,
        name: newName.trim(),
  ...
  const deleteMember = async (id: string) => {
    try {
      const { error } = await supabase.from("vendor_staff_roster").delete().eq("id", id);
      if (error) throw error;
  ...
  const addRole = async () => {
    ...
    setAddingRole(true);
    try {
      const { error } = await supabase.from("vendor_staff_roles").insert({
        vendor_id: vendorId,
  ...
  const deleteRole = async (id: string) => {
    try {
      const { error } = await supabase.from("vendor_staff_roles").delete().eq("id", id);
      if (error) throw error;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Save your go-to staff and roles here to keep track of your team.
        </p>
      </div>
      ...
      <Tabs defaultValue="people">
        ...
        <TabsContent value="roles" className="mt-4 space-y-4">
          ...
          <div className="flex gap-2">
            <Input
              placeholder="New role name (e.g. Cashier, Setup, Sales)..."
              value={newRoleName}
          ...
          {roles.length === 0 ? (
            <Card className="border-dashed">
              <CardContent className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  No roles saved yet. Add common roles like Cashier, Setup/Teardown, or Sales above.
                </p>
              </CardContent>
            </Card>
          ) : (
...
export default VendorStaffRoster;
